...
src/
  ...
  lib/
    camera/
      stream.ts            # getUserMedia (выбор камеры), остановка потока, проба на чёрный кадр
    pose/
...
Рабочий режим — задняя камера (`facingMode: "environment"`). Телефон кладётся на пол горизонтально сбоку от пользователя так, чтобы в кадр попадало тело целиком в профиль.

Есть переключатель на фронтальную камеру (`facingMode: "user"`). Он нужен для отладки: с задней камерой пользователь не видит экран и не может проверить, что скелет ложится на тело. Фронтальная картинка отзеркаливается — один и тот же CSS-трансформ вешается на видео и на overlay, поэтому landmarks остаются выровнены и пересчёт координат не нужен. По умолчанию всегда задняя.
